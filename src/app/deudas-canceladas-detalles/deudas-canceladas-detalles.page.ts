...
export class DeudasCanceladasDetallesPage implements OnInit {
  ...
  ionViewWillEnter() {
    console.log('DeudasCanceladasDetallesPage - ionViewWillEnter - Cargando datos para deuda ID:', this.idrecibido);
    this.cargando = true;
  ...
  precalcularDatos() {
    ...
    console.log('DeudasCanceladasDetallesPage - precalcularDatos - Procesando datos');
    ...
    // Fecha de cancelación (último registro)
    const ultimoRegistro = this.historiales[this.historiales.length - 1];
    this.fechaCancelacion = ultimoRegistro.fechas || '';

    console.log('DeudasCanceladasDetallesPage - precalcularDatos - Datos procesados:', {
  ...
  crearBoletaParaCaptura(): HTMLElement {
    ...
    const header = document.createElement('div');
    header.style.cssText = `
      border-bottom: 2px solid #28ba62;
      padding-bottom: 10px;
      margin-bottom: 10px;
    `;
    header.innerHTML = `
      <div style="text-align: center; font-size: 1.2rem; font-weight: 700; color: #28ba62; margin-bottom: 8px;">
        ESTADO DE CUENTA - CANCELADA
      </div>
    ...
    footer.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
        ...
        <span style="font-size: 1.2rem; font-weight: 700; color: #28ba62;">
          Gs. 0
        </span>
      </div>
      <div style="text-align: center; margin-top: 8px;">
        <span style="display: inline-block; background: #28ba62; color: white; padding: 5px 12px; border-radius: 15px; font-size: 0.75rem; font-weight: 600;">
          ✓ DEUDA CANCELADA
        </span>
      </div>
  ...
  async tomarScreen() {
    ...
    try {
      ...
      const fileName = `deuda_cancelada_${Date.now()}.png`;
      ...
      await Share.share({
        title: 'Detalle de Deuda Cancelada',
        url: savedFile.uri,
  ...
  async generatePdf() {
    ...
    try {
      ...
      const fileName = `deuda_cancelada_${Date.now()}.pdf`;
      const savedFile = await Filesystem.writeFile({
        path: fileName,
        data: base64,
        directory: Directory.Cache
      ...
      await Share.share({
        title: 'Reporte de Deuda Cancelada',
        url: savedFile.uri,
